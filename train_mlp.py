...
from model.mlp import create_mlp_model


class CombinedTerrainDataset(Dataset):
    """
    合并多个TerrainDataset的数据集类，支持逐列归一化
    """
    ...
    #     print(f"特征归一化统计:")
    #     print(f"  - 均值范围: [{self.feature_means.min():.4f}, {self.feature_means.max():.4f}]")
    #     print(f"  - 标准差范围: [{self.feature_stds.min():.4f}, {self.feature_stds.max():.4f}]")
    #     print(f"  - 特征维度: {len(self.feature_means)}")
...
def train_classification_model():
    """
    训练MLP分类模型（全量数据，包含训练集和验证集）
    """
    print("开始训练MLP分类模型（全量数据）...")
    ...
    model = create_mlp_model(input_size=input_size, output_size=output_size)
    
    # 5. 定义损失函数和优化器（分类任务使用CrossEntropyLoss）
    ...
    model_save_path = "./weight/trained_mlp_classifier.pth"
    ...
    with open('./weight/mlp_test_results.json', 'w') as f:
    ...
    with open('./weight/mlp_training_history.json', 'w') as f:
        json.dump(training_history, f, indent=2)
    
    print(f"训练完成！模型已保存到: {model_save_path}")
    print(f"测试结果已保存到: ./weight/mlp_test_results.json")
    print(f"训练历史已保存到: ./weight/mlp_training_history.json")
...
if __name__ == "__main__":
    ...
    # 开始训练
    model, train_losses, train_accuracies, val_accuracies = train_classification_model()
    ...
    if model is not None:
        ...
        print(f"- 最终训练准确率: {train_accuracies[-1]:.2f}%")
        print(f"- 最终验证准确率: {val_accuracies[-1]:.2f}%")
        print(f"- 模型已保存为: trained_mlp_classifier.pth")
        
        # 可选：加载并评估模型
        print("\n加载模型进行评估...")
        try:
            checkpoint = torch.load("./weight/trained_mlp_classifier.pth")
